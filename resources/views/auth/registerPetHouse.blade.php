...
<div class="w-screen min-h-screen bg-linear-30 from-orange-500 to-red-400">
    <div class="px-5 w-full md:w-2/3 xl:w-1/3 absolute top-1/2 left-1/2 -translate-1/2 backdrop-blur-sm flex flex-col gap-y-5">
        <p class="text-center text-xl font-semibold">Register</p>
        <form method="POST" action="{{ route('register.pethouse.store') }}">
            @csrf
            <div class="mb-5">
            ...
            <div class="mb-5">
                <label for="repeat-password" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Repeat
                    password</label>
                <input type="password" id="repeat-password" name="confirmPassword"
                    class="shadow-xs bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500 dark:shadow-xs-light"
                    required />
            </div>
            <div class="flex items-start mb-5">
                <div class="flex items-center h-5">
                    <input id="terms" type="checkbox" value="{{ true }}" name="tersm and condition"
